...
# guard armed) via scripts/arasaka-finalize-install.sh. It then packs the
# installed rootfs into a raw squashfs + dm-verity slot image (the same tool
# the OTA bundle builder uses) and images the inactive slot B with it, making
# slot B the active slot so the very first boot of the installed system is
# already block-verified. The systemd-boot entries are left to the Calamares
# bootloader module (they reference the root by UUID); the A/B loader entries
# are generated by the running system's update machinery once the installed
# layout is known.
#
